...
real(4) :: u_to,u_to1,u_to2,re,xl2,xl3,xnu,x14,x15,pr,alpha,dxp,dzp,Re_tau
real(4) :: xlx=15.70796,zlz=6.283185,Gr=960000
real(8),dimension(ny1,77) :: q_stat
...
re=3510!10322.629 !Creo qeu es el reynold usadopost_log_ut_budget.txt
pr=0.71
alpha=xnu/pr
...
do j=1,ny1
    q_stat(j,69)=-0.5*(q_stat(j,77)-q_stat(j,2)*q_stat(j,8)-2*q_stat(j,7)*q_stat(j,10))/(u_to*x15*x15)    !vtt
enddo
...
do j=1,ny1
   q_stat(j,76)=-1000*(q_stat(j,73)+q_stat(j,75)-q_stat(j,1)*q_stat(j,74)-q_stat(j,2)*q_stat(j,58))/(u_to**3*xl2)   !Velocity pressure-gradient correlation
enddo
...
do j=1,ny1
   ...
   q_stat(j,38)=q_stat(j,10)*q_stat(j,41)*xnu/(x15*u_to)   !production
   q_stat(j,20)=-(Gr/(2*xl2)**3)*q_stat(j,10)*x15       !Production by buoyancy
   q_stat(j,50)=-(1/pr)*(q_stat(j,40)+q_stat(j,42)+q_stat(j,44)- &
                           q_stat(j,39)*q_stat(j,39)-q_stat(j,41)*q_stat(j,41)- &
                           q_stat(j,43)*q_stat(j,43))/(x15**2*xl2**2)           !Dissipation
...
do j=1,ny1-1
   q_stat(j,67)=(1/(2*pr))*(q_stat(j+1,8)**2-q_stat(j,8)**2)/((yp(j+1)-yp(j))*xl2)
enddo

do j=1,ny1-2
   q_stat(j,67)=(q_stat(j+1,67)-q_stat(j,67))/((ypi(j+1)-ypi(j))*xl2)              !Molecular diffusion
enddo
...
!100 format(13F12.6)
  ...
  open (144,file='Flow_budget_tt.dat',form='formatted',status='unknown')
  do j=1,ny1/2
      write(144,100) yp(j),yp(j)*xl2,q_stat(j,38),q_stat(j,20),q_stat(j,50),q_stat(j,76)
   enddo
   close(144)

  open (144,file='Flow_budget2_tt.dat',form='formatted',status='unknown')
  do j=1,ny1/2
      write(144,100) ypi(j),ypi(j)*xl2,q_stat(j,69)
   enddo
   close(144)

  open (144,file='Flow_budget3_tt.dat',form='formatted',status='unknown')
  do j=1,ny1/2
      write(144,100) ypii(j),ypii(j)*xl2,q_stat(j,67)
   enddo
   close(144)
