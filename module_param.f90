...
integer,parameter :: p_row=4,p_col=64
integer,parameter :: nxm=nx,nym=ny-1,nzm=nz 
!end module variables

real(mytype), parameter :: gravity=9.8
real(mytype), parameter :: Betaexp=0.001434397
real(mytype), parameter :: DeltaT=1.0
...
!module scalar                                                         !Guo
real(mytype), dimension(nx) :: sfxt,scxt,sbxt,ssxt,swxt                !Guo
real(mytype), dimension(nx) :: sfxpt,ssxpt,swxpt                       !Guo 
real(mytype), dimension(ny) :: sfyt,scyt,sbyt,ssyt,swyt                !Guo
real(mytype), dimension(ny) :: sfypt,ssypt,swypt                       !Guo
real(mytype), dimension(nz) :: sfzt,sczt,sbzt,sszt,swzt                !Guo 
real(mytype), dimension(nz) :: sfzpt,sszpt,swzpt                       !Guo

!module implicit                                                       !Guo
real(mytype), dimension(ny) :: aam,bbm,ccm,ddm,eem,ggm,hhm,wwm,zzm !!TIME IMPLICIT, ncl=2                               !Guo    
real(mytype), dimension(ny) :: rrm,qqm,vvm,ssm !!TIME IMPLICIT (with HPL), ncl=2                                        !Guo
real(mytype), dimension(ny) :: aam10,bbm10,ccm10,ddm10,eem10,ggm10,hhm10,wwm10,zzm10 !!TIME IMPLICIT, ncl=1, npaire=0   !Guo
real(mytype), dimension(ny) :: rrm10,qqm10,vvm10,ssm10 !!TIME IMPLICIT (with HPL), ncl=1, npaire=0                      !Guo 
real(mytype), dimension(ny) :: aam11,bbm11,ccm11,ddm11,eem11,ggm11,hhm11,wwm11,zzm11 !!TIME IMPLICIT, ncl=1, npaire=1   !Guo
real(mytype), dimension(ny) :: rrm11,qqm11,vvm11,ssm11 !!TIME IMPLICIT (with HPL), ncl=1, npaire=1                      !Guo
real(mytype), dimension(ny) :: aam0,bbm0,ccm0,ddm0,eem0,ggm0,hhm0,wwm0,zzm0 !!TIME IMPLICIT, ncl=0                      !Guo 
real(mytype), dimension(ny) :: rrm0,qqm0,vvm0,ssm0,l1m,l2m,l3m,u1m,u2m,u3m !!TIME IMPLICIT (with HPL), ncl=0            !Guo
real(mytype), dimension(ny) :: aamt,bbmt,ccmt,ddmt,eemt,ggmt,hhmt,wwmt,zzmt !!TIME IMPLICIT SCALAR, ncl=2               !Guo
real(mytype), dimension(ny) :: rrmt,qqmt,vvmt,ssmt !!TIME IMPLICIT SCALAR (with HPL), ncl=2                             !Guo
real(mytype), dimension(ny) :: aamt1,bbmt1,ccmt1,ddmt1,eemt1,ggmt1,hhmt1,wwmt1,zzmt1 !!TIME IMPLICIT SCALAR, ncl=1      !Guo
real(mytype), dimension(ny) :: rrmt1,qqmt1,vvmt1,ssmt1 !!TIME IMPLICIT SCALAR (with HPL), ncl=1                         !Guo 
real(mytype), dimension(ny) :: aamt0,bbmt0,ccmt0,ddmt0,eemt0,ggmt0,hhmt0,wwmt0,zzmt0 !!TIME IMPLICIT SCALAR, ncl=0      !Guo
real(mytype), dimension(ny) :: rrmt0,qqmt0,vvmt0,ssmt0,l1mt,l2mt,l3mt,u1mt,u2mt,u3mt !!TIME IMPLICIT SCALAR (with HPL), ncl=0  !Guo
...
use decomp_2d, only : mytype
  ...
  integer, save :: iimplicit !!TIME IMPLICIT            !Guo
  real(mytype) :: xcst, xcst_pr !!TIME IMPLICIT         !Guo
  !!                                                    !Guo
  !! Robin boundary condition on temperature            !Guo
  !! alpha * T + beta * dT/dn = g                       !Guo
  !! alpha=1, beta=0 is dirichlet                       !Guo
  !! alpha=0, beta=1 is neumann                         !Guo
  !!                                                    !Guo
  !! WARNING ATTENTION ACHTUNG WARNING ATTENTION ACHTUNG   !Guo
  !!                                                       !Guo 
  !! beta is the coefficient for NORMAL derivative :       !Guo
  !!                                                       !Guo 
  !! alpha_0*T(0) - beta_0*dTdy(0)=g_0                     !Guo
  !! alpha_n*T(L) + beta_n*dTdy(L)=g_n                     !Guo
  !!                                                       !Guo
  !! WARNING ATTENTION ACHTUNG WARNING ATTENTION ACHTUNG   !Guo
  !!                                                       !Guo
  real(mytype) :: alpha_0, beta_0, g_0, alpha_n, beta_n, g_n  !Guo
end module param
...
use decomp_2d, only : mytype
  ...
  real(mytype) :: ailcaix6,aicix6,bicix6,cicix6,dicix6              !Guo
  ...
  real(mytype) :: alsaixt,asixt,bsixt,csixt                 !Guo 
end module derivX
...
use decomp_2d, only : mytype
  ...
  real(mytype) :: ailcaiy6,aiciy6,biciy6,ciciy6,diciy6              !Guo   
  ...
  real(mytype) :: alsajyt,asjyt,bsjyt,csjyt                 !Guo
end module derivY
...
use decomp_2d, only : mytype
  ...
  real(mytype) :: ailcaiz6,aiciz6,biciz6,ciciz6,diciz6              !Guo
  ...
  real(mytype) :: alsakzt,askzt,bskzt,cskzt                !Guo
end module derivZ
