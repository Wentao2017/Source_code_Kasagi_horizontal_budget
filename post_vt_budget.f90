...
do j=1,ny1
   ...
   q_stat(j,20)=-Gr_plus*q_stat(j,8)**2        !Production by buoyancy
...
!100 format(13F12.6)
  ...
  open (144,file='Flow_budget_vt.dat',form='formatted',status='unknown')
  do j=1,ny1/2
      write(144,100) yp(j),yp(j)*xl2,q_stat(j,38),q_stat(j,20),q_stat(j,50),q_stat(j,59)
   enddo
   close(144)

  open (144,file='Flow_budget2_vt.dat',form='formatted',status='unknown')
  do j=1,ny1/2
      write(144,100) ypi(j),ypi(j)*xl2,q_stat(j,56),q_stat(j,66)
   enddo
   close(144)

  open (144,file='Flow_budget3_vt.dat',form='formatted',status='unknown')
  do j=1,ny1/2
