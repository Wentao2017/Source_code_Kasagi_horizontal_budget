...
!100 format(13F12.6)
  ...
  open (144,file='Flow_budget_ut.dat',form='formatted',status='unknown')
  do j=1,ny1/2
      write(144,100) yp(j),yp(j)*xl2,q_stat(j,38),q_stat(j,50),q_stat(j,59)
   enddo
   close(144)

  open (144,file='Flow_budget2_ut.dat',form='formatted',status='unknown')
  do j=1,ny1/2
      write(144,100) ypi(j),ypi(j)*xl2,q_stat(j,56),q_stat(j,66)
   enddo
   close(144)

  open (144,file='Flow_budget3_ut.dat',form='formatted',status='unknown')
  do j=1,ny1/2
