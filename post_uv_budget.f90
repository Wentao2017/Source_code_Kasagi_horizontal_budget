...
real(8),dimension(ny1,76) :: q_stat
...
do j=1,ny1
   ...
   q_stat(j,20)=Gr_plus*q_stat(j,9)       !Production by buoyancy
...
do j=1,ny1-1
   q_stat(j,67)=(q_stat(j+1,11)-q_stat(j,11))/((yp(j+1)-yp(j))*xl2)
enddo
...
!100 format(13F12.6)
  ...
  open (144,file='Flow_budget_uv.dat',form='formatted',status='unknown')
  do j=1,ny1/2
      write(144,100) yp(j),yp(j)*xl2,q_stat(j,38),q_stat(j,20),q_stat(j,50),q_stat(j,76)
   enddo
   close(144)

  open (144,file='Flow_budget2_uv.dat',form='formatted',status='unknown')
  do j=1,ny1/2
      write(144,100) ypi(j),ypi(j)*xl2,q_stat(j,69)
   enddo
   close(144)

  open (144,file='Flow_budget3_uv.dat',form='formatted',status='unknown')
  do j=1,ny1/2
